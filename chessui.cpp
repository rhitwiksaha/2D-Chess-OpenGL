...
void skeleton_box(int x, int y)		//display the border of each squares
{
	glColor3f(0.6078, 0.3019, 0.0745);
	glLineWidth(3);
	rect_box(x*d + offset, y*d + offset, (x + 1)*d + offset, (y + 1)*d + offset);
}
void clearbox(int x, int y)			//clear (x,y) box in chess board -- fills the square with colors
{
	if ((x + y) % 2 == 0)
...
void mouse(int b, int s, int x, int y)
{
	...
	// printf("%c%d\n",x+'a',y+1);
	glutTimerFunc(185, reset, 0);		//mouse is re-enabled after 185ms
}
